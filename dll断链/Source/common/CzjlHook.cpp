...
/*
			...
			int oldptr = SearchPos;
			SearchPos = (DWORD)st.BaseAddress + st.RegionSize;
			if(SearchPos <= (unsigned int)oldptr)
				break;
		}
...
unsigned int CzjlHook::FindMemTargetFromSection( char* pStart,char* pTag,unsigned int LenTag )
{
	...
	printf("FindMemTargetFromSection  0x%08x -  0x%08x\r\n Lend=0x%08x",CodeStart,CodeEnd,CodeSize);
	if (CodeSize==0)
		return 0;
...
//zjlDebugBoxOutHex("SIZE",(int)st.RegionSize);
		...
		int oldptr = Address;
		Address = (DWORD)st.BaseAddress + st.RegionSize;
		if(Address <= (unsigned int)oldptr)
			break;
	}
	return 0;
}
		...
		while(searchPos<CodeEnd-5)
		{
			//JMP偏移
			 ...
			 {//E9
				...
				{
					//DbgPrint("FindHookAddr : 0x%08x,JmpTo : 0x%08x  ",searchPos,CmpAddr+searchPos);
				}
			 }
