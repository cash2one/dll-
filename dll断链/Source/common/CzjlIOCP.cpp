...
bool Initialize()
{
	...
	g_nThreads =1;//WORKER_THREADS_PER_PROCESSOR * GetNoOfProcessors();
...
//	InitializeCriticalSection(&g_csConsole);
	...
	if (false == InitializeIOCP())//初始化工作线程
	{
...
DWORD WINAPI AcceptThread(LPVOID lParam)
{
	...
	while(WAIT_OBJECT_0 != WaitForSingleObject(g_hShutdownEvent, 0))//检查结束
	{
...
void AcceptConnection(SOCKET ListenSocket)
{
	...
	if (true == AssociateWithIOCP(pClientContext))//创建完成端口
	{
...
{    
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(g_hShutdownEvent, 0))
	{
		...
		case OP_WRITE:
			...
			//WriteToConsole("\nThread %d:收到客户端的信息: %s", nThreadNo, szBuffer);
