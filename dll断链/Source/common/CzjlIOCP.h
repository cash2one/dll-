...
#define WAIT_TIMEOUT_INTERVAL 0
...
//for shutdown
HANDLE g_hShutdownEvent = NULL; //暂停标记

//Number of threads to be created.
int g_nThreads = 0;     //工作线程数
...
bool InitializeIOCP();
void CleanUp();//清理
void DeInitialize();//结束

DWORD WINAPI AcceptThread(LPVOID lParam);
DWORD WINAPI WorkerThread(LPVOID lpParam);

void AcceptConnection(SOCKET ListenSocket);     //监听线程
bool AssociateWithIOCP(CClientContext   *pClientContext);   //创建此连接的完成端口

//void WriteToConsole(char *szFormat, ...);

void AddToClientList(CClientContext   *pClientContext);     //List增加项
...
class CzjlIOCP
{
    bool ServerInit(unsigned int nWorkThread=0,unsigned int nMaxConnect=0);
    bool ServerStart(unsigned int nPort=9527);
    void ServerEnd();

    bool ClientInit(char* pIP,unsigned int nPort);
    bool ClientEnd();
